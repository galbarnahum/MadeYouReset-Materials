...
final class HTTP1TestServer: ChannelInboundHandler {
    ...
    public func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        // Insert a 500ms delay before sending the response
        context.eventLoop.scheduleTask(in: .milliseconds(500)) {
            context.channel.getOption(HTTP2StreamChannelOptions.streamID).flatMap { (streamID) -> EventLoopFuture<Void> in
                ...
                let writeHead = context.channel.write(self.wrapOutboundOut(
                ...
                let writeBody = context.channel.write(self.wrapOutboundOut(
                    HTTPServerResponsePart.body(.byteBuffer(buffer))
                ))

                let writeEnd = context.channel.writeAndFlush(self.wrapOutboundOut(HTTPServerResponsePart.end(nil)))

                return writeHead.and(writeBody).and(writeEnd).map { _ in () }
            }.whenComplete { _ in
...
final class ErrorHandler: ChannelInboundHandler, Sendable {
    ...
    func errorCaught(context: ChannelHandlerContext, error: Error) {
        if let h2Error = error as? NIOHTTP2Errors.StreamClosed {
            print("Ignoring stream closed error: \(h2Error)")
            return // Do not close the connection for a stream error
        }

        if let h2FlowControlError = error as? NIOHTTP2Errors.InvalidFlowControlWindowSize {
            print("Flow control error: \(h2FlowControlError), but continuing")
            return // Ignore flow control window errors for now
        }

        // Log other errors and close the connection only if necessary
